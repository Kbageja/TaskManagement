...
const DashboardLayout = ({ children }: LayoutProps) => {
  ...
  console.log(data,"groupLevelWise")
...
const handleTaskSubmit = (e: React.FormEvent) => {
  ...
  if (selectedGroup && selectedUser && taskTitle && taskDeadline && taskPriority) {
...
const resetForm = () => {
      ...
      <div className="w-16 bg-blue-800 text-white">
        ...
        <nav className="mt-20 flex-col justify-center align-center">
          <div className="space-y-6">
            ...
            <a href="#" className="flex items-center space-x-2 px-4 py-4 hover:bg-blue-600 rounded" onClick={(e) => {
              e.preventDefault();
  ...
  {filteredMembers
    .filter((member) => {
      // Get the current user's level from localStorage
      const userId = localStorage.getItem('userId') || ''; // Fallback to empty string if userId is null
      const currentUser = data?.Data?.find(
        (group) => group.id === parseInt(selectedGroup)
      )?.members?.find((member) => member.id === parseInt(userId || '0')); // Fallback to '0' if userId is empty

      // Get the current user's level
      const currentUserLevel = currentUser?.level || 0;

      // Only show users with a level greater than the current user's level
